...
// Adapted from https://github.com/palantir/osdk-ts/blob/876632789fb7af0e084291eb5dbb0bd610f35cc5/packages/client/src/internal/conversions/modernToLegacyWhereClause.ts#L41
// We should either expose the normalized where clause, or even better expose the normalized object set.
...
import type { PropertyIdentifier, SearchJsonQueryV2 } from "@osdk/internal.foundry.core";
import type { BBox, Position } from "geojson";
import invariant from "tiny-invariant";

/** @internal */
export function modernToLegacyWhereClause<T extends ObjectOrInterfaceDefinition>(
    ...
    if ("$and" in whereClause) {
        return {
            type: "and",
            value: (whereClause.$and as WhereClause<T>[]).map((clause) =>
                modernToLegacyWhereClause(clause, objectOrInterface)
            ),
        };
    } else if ("$or" in whereClause) {
        return {
            type: "or",
            value: (whereClause.$or as WhereClause<T>[]).map((clause) =>
                modernToLegacyWhereClause(clause, objectOrInterface)
            ),
        };
        ...
        return {
            type: "not",
            value: modernToLegacyWhereClause(whereClause.$not as WhereClause<T>, objectOrInterface),
        };
    }
    ...
    return {
        type: "and",
        value: parts.map<SearchJsonQueryV2>((v) => handleWherePair(v, objectOrInterface)),
    };
}
...
function handleWherePair(
    ...
    const propertyIdentifier: PropertyIdentifier | undefined =
        structFieldSelector != null
            ...
            : undefined;
    const field =
        structFieldSelector == null ? fullyQualifyPropName(fieldName, objectOrInterface) : undefined;
    ...
    if (firstKey === "$intersects") {
        ...
        if (Array.isArray(intersectsBody)) {
            ...
            const coordinates =
                "$polygon" in intersectsBody ? intersectsBody.$polygon : intersectsBody.coordinates;
...
type DropDollarSign<T extends `$${string}`> = T extends `$${infer U}` ? U : never;
...
function fullyQualifyPropName(fieldName: string, objectOrInterface: ObjectOrInterfaceDefinition) {
